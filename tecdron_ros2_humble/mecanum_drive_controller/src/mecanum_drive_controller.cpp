...
controller_interface::CallbackReturn MecanumDriveController::on_init()
{
  try
  {
    ...
    auto_declare<std::string>("filtered_odometry_topic", "/true_odom");
                                                                 ...
                                                                 const rclcpp::Duration& period)
{
  ...
  /*Get the odometry in order to update the controller*/

  std::shared_ptr<nav_msgs::msg::Odometry> filtered_odometry;
  received_filtered_msg_ptr_.get(filtered_odometry);

  if (filtered_odometry == nullptr)
  {
    RCLCPP_WARN(logger, "Odometry message received was a nullptr.");
    return controller_interface::return_type::ERROR;
  }
  ...
  measured_states << filtered_odometry->twist.twist.linear.x,
                     filtered_odometry->twist.twist.linear.y,
                     filtered_odometry->twist.twist.angular.z;

  commands << linear_command_x,
              linear_command_y,
              angular_command;
  ...
  /*Saving the parameters for the next step*/
  X_k = X_k1;
  error_k = error_k1;
  last_time = now;
  
  const double velocity_front_left = Y(0);
  const double velocity_front_right = Y(1);
  const double velocity_rear_right = Y(2);
  const double velocity_rear_left = Y(3);
...
controller_interface::CallbackReturn MecanumDriveController::on_configure(const rclcpp_lifecycle::State&)
{
  ...
  filtered_odometry_topic_ = get_node()->get_parameter("filtered_odometry_topic").as_string();

  if (filtered_odometry_topic_.empty())
  {
    RCLCPP_ERROR(logger, "This code requires a filtered odometry topic to be set");
    return controller_interface::CallbackReturn::ERROR;
  }
  ...
  filtered_odometry_subscriber_ = get_node()->create_subscription<nav_msgs::msg::Odometry>(
        filtered_odometry_topic_, rclcpp::SystemDefaultsQoS(),
        [this](const std::shared_ptr<nav_msgs::msg::Odometry> msg) -> void {
          ...
          {
            RCLCPP_WARN_ONCE(get_node()->get_logger(),
                             "Received Filterd Odometry with zero timestamp, setting it to current "
                             "time, this message will only be shown once");
            msg->header.stamp = get_node()->get_clock()->now();
          }
          received_filtered_msg_ptr_.set(std::move(msg));
...
bool MecanumDriveController::reset()
{
  ...
  received_filtered_msg_ptr_.set(nullptr);
  is_halted = false;
  return true;
}
